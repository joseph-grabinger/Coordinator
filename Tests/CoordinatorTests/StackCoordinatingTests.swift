...
@Suite("Stack Coordinating Tests") struct StackCoordinatingTests {
    ...
    @Test func testInit() throws {
        ...
        let sharedPathManager = SharedPathManager(coordinator: sut)

        // THEN: The SUT's root is set.
        #expect(sut.root != nil, "Root is expected to be non-nil")
        let root = try #require(sut.root as? SharedPathManager<MockRoute>, "Root is expected to have type SharedPathManager")
        #expect(root.initialRoute == sharedPathManager.initialRoute, "Initial routes are expected to match")
    }
    ...
    @Test func testInitPath() throws {
        ...
        // THEN: The root coordinator's path matches the SUT's pat
        let root = try #require(sut.root as? SharedPathManager<MockRoute>, "Root is expected to have type SharedPathManager")
        #expect(sut.presentedRoutes.count == root.path.count, "SUT and root path's are expected to be equal")
        #expect(root.initialRoute == sharedPathManager.initialRoute, "Initial routes are expected to match")
    }
    ...
    @Test func testPushCoordinatorSuccess() {
        ...
        #expect(sharedPathManager.path.count == 1, "Root's path is expected to contain one element")
    ...
    @Test func testPopRouteSuccess() {
        ...
        #expect(sharedPathManager.path.count == path.count, "Root path is expected to equal the initial path")
        ...
        #expect(sut.presentedRoutes.count == 1, "SUT path is expected to have one element")
        #expect(sharedPathManager.path.count == 1, "Root path is expected to have one element")

        // AND: The paths are still equal
        #expect(sharedPathManager.path.count == sut.presentedRoutes.count, "Root path is expected to match the SUT's path")
    }
    ...
    @Test func testPopToRootSuccessNonEmptyPath() {
        ...
        #expect(sut.presentedRoutes.count == root.path.count, "The SUT path and root path are expected to be equal")
    }
